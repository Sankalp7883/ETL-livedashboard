...
def transform_dataset(df: pd.DataFrame, output_table: str):
    ...
    # 4️⃣ Convert timestamp-like numeric columns to readable dates
    for col in df.columns:
        if df[col].dtype in [np.int64, np.float64]:
            if df[col].between(1e12, 2e12).any():  # timestamp in ns
                try:
    ...
    # 5️⃣ Convert date-like text columns
    for col in df.columns:
        if "date" in col.lower() or "time" in col.lower():
            df[col] = pd.to_datetime(df[col], errors='coerce')

    # 6️⃣ Replace infinities with NaN
    df = df.replace([np.inf, -np.inf], np.nan)

    # 7️⃣ Safe string cleanup for text columns
    for col in df.columns:
